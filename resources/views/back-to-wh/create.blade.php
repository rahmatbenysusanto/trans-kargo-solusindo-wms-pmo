...
@section('title', 'Create Back To WH')

@section('content')
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Create Back To Warehouse</h4>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center">
                            <div class="avatar-xs me-3">
                                <div class="avatar-title bg-teal-subtle text-teal rounded-circle"
                                    style="color: #0d9488; background-color: rgba(13, 148, 136, 0.1);">
                                    <i class="ri-archive-drawer-line fs-18"></i>
                                </div>
                            </div>
                            <h5 class="card-title mb-0">Back To Warehouse Information</h5>
                        </div>
                        <button class="btn btn-primary btn-label waves-effect waves-light" onclick="createBackToWh()">
                            <i class="ri-send-plane-2-line label-icon align-middle fs-16 me-2"></i> Submit Back To WH
                        </button>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label text-muted small text-uppercase">Return Reason <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <span class="input-group-text bg-light border-light"><i class="ri-question-answer-line"></i></span>
                                <textarea class="form-control border-light bg-light" id="reason" rows="2"
                                    placeholder="Why are these items returning to warehouse?"></textarea>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label text-muted small text-uppercase">Received Date <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <span class="input-group-text bg-light border-light"><i class="ri-calendar-event-line"></i></span>
                                <input type="date" class="form-control border-light bg-light" id="receivedAt"
                                    value="{{ date('Y-m-d') }}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <span class="input-group-text bg-light border-light"><i class="ri-user-follow-line"></i></span>
                        ...
                        <div class="col-md-3">
                            <label class="form-label text-muted small text-uppercase">Total Items</label>
                            <div class="input-group">
                                <span class="input-group-text bg-teal text-white border-teal"
                                    style="background-color: #0d9488; border-color: #0d9488;"><i class="ri-hand-coin-line"></i></span>
                                <input type="number" class="form-control border-teal bg-teal-subtle fw-bold"
                                    style="border-color: #0d9488; background-color: rgba(13, 148, 136, 0.05);"
                ...
                <div class="card-header bg-light-subtle py-3 border-bottom">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <h5 class="card-title mb-0">Inventory in Use (Outbound)</h5>
                            <small class="text-muted">Items that have been sent out and are eligible for return</small>
                        </div>
                    </div>
                </div>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-light-subtle py-3 border-bottom">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <h5 class="card-title mb-0 text-success">Selected for Back to WH</h5>
                        </div>
                    </div>
                </div>
                <div class="card-body p-0">
                    ...
                    <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                        <table id="tableProductsBackToWh" class="table table-hover table-nowrap align-middle mb-0">
...
@section('js')
    ...
    <script>
        ...
        function fetchInventory(page = 1) {
            currentPage = page;
            $.ajax({
                url: '{{ route('backToWh.inventory.search') }}',
                method: 'GET',
                data: {
                    search: searchQuery,
                    page: page
        ...
        function processBulkSn(btn) {
            ...
            $.ajax({
                url: '{{ route('backToWh.inventory.searchBySN') }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    serial_numbers: sns
                ...
                success: function(res) {
                    ...
                    if (found.length > 0) {
                        const products = JSON.parse(localStorage.getItem('productsBackToWh')) ?? [];
                        ...
                        localStorage.setItem('productsBackToWh', JSON.stringify(products));
                        viewProducts();
                    ...
                    if (found.length > 0) {
                        resultHtml +=
                            `<div class="alert alert-success py-2 mb-1 small"><i class="ri-check-line me-1"></i>${found.length} SN(s) ditemukan & ditambahkan</div>`;
                    }
                    if (notFound.length > 0) {
                        resultHtml +=
                            `<div class="alert alert-warning py-2 mb-1 small"><i class="ri-error-warning-line me-1"></i>Tidak ditemukan: <strong>${notFound.join(', ')}</strong></div>`;
                    }
        ...
        function viewProducts() {
                        ...
                        <tr>
                            ...
                            <td>
                                <select class="form-select form-select-sm border-light bg-light"
                                    onchange="updateProductCondition(${index}, this.value)" style="min-width: 90px;">
                                    <option value="Good" ${product.condition === 'Good' ? 'selected' : ''}>Good</option>
                                    <option value="Scrape" ${product.condition === 'Scrape' ? 'selected' : ''}>Scrape</option>
                                    <option value="Damage" ${product.condition === 'Damage' ? 'selected' : ''}>Damage</option>
                                </select>
                            </td>
                            <td>
                                <input type="text" class="form-control form-control-sm border-light bg-light"
            ...
            document.getElementById('qtyProduct').value = products.length;
            document.getElementById('productsBackToWh').innerHTML = html;
        }
        ...
        function createBackToWh() {
            const reason = document.getElementById('reason').value;
            if (!reason) {
                Swal.fire('Error', 'Please enter the return reason', 'error');
                return;
            }

            const products = JSON.parse(localStorage.getItem('productsBackToWh')) ?? [];
            if (products.length === 0) {
                Swal.fire('Error', 'Please select at least 1 product', 'error');
                return;
            }

            Swal.fire({
                title: "Are you sure?",
                text: "Create Back To Warehouse with " + products.length + " item(s)",
                icon: "warning",
                ...
                if (t.value) {
                    $.ajax({
                        ...
                        data: {
                            ...
                            products: JSON.parse(localStorage.getItem('productsBackToWh')) ?? []
                        },
                        success: (res) => {
                            if (res.status) {
                                Swal.fire({
                                    title: 'Success',
                                    text: 'Back To Warehouse created successfully',
                                    icon: 'success'
                                }).then((i) => {
                                ...
                                Swal.fire({
                                    title: 'Error',
                                    text: 'Failed to create Back To Warehouse',
                                    icon: 'error'
                                });
                            }
                        }
